  ...
  return (
    <div
      ...
      className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer ${
        active ? "bg-primary" : "hover:bg-primary/20"
      } transition-colors`}
    >
      <Icon className={`w-6 h-6 ${active ? "text-black" : "text-white/70"}`} />
      <p
        className={`font-display ${
          active ? "text-black font-bold" : "text-white/90"
        }`}
      >
        {label}
      </p>
...
function Navbar({ className, ...props }: NavbarProps) {
  ...
  const signOutResult = await handleSignOut(email, password);

  return (
    <nav
      className={`relative bg-primary-dark flex flex-col h-full md:border-r-[0.5px] border-white/15 ${
        className ?? ""
      }`.trim()}
      {...props}
    >
        ...
        <div className="flex items-center">
          ...
          {!loading && !user ? (
            ...
            <div className="flex items-center gap-3 mr-10">
              <div className="size-10 bg-white/15 p-1.75 rounded-3xl">
